...
function loadCategories() {
    ...
    database.ref('Categories2').orderByChild('order').once('value', (snapshot) => {
...
function updateCategoryOrder(evt) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        database.ref('Categories2/' + categoryId).update({
            order: i
        });
    }
}
...
document.getElementById('newCategoryForm').addEventListener('submit', function(e) {
    ...
    resizeAndCompressImage(categoryImageFile, 800, 800, 0.7, function(dataUrl) {
        ...
        const storageRef = storage.ref('categoryImages2/' + categoryImageFile.name);
        ...
        uploadTask.on('state_changed',
            ...
            () => {
                uploadTask.snapshot.ref.getDownloadURL().then((url) => {
                    ...
                    database.ref('Categories2').once('value').then((snapshot) => {
                        const categoryCount = snapshot.numChildren();
                        const newCategoryKey = database.ref().child('Categories2').push().key;
                        database.ref('Categories2/' + newCategoryKey).set({
                            name_tr: name_tr,
                            imageUrl: url,
...
function deleteCategory(categoryId) {
    ...
    if (confirmDelete) {
        ...
        database.ref('Products2').orderByChild('categoryId').equalTo(categoryId).once('value', (snapshot) => {
            snapshot.forEach((childSnapshot) => {
                database.ref('Products2/' + childSnapshot.key).remove();
            });

            // Kategoriyi sil
            database.ref('Categories2/' + categoryId).remove()
                .then(() => {
...
function openUpdateCategoryModal(categoryId) {
    ...
    database.ref('Categories2/' + categoryId).once('value', (snapshot) => {
...
function loadProducts() {
    ...
    database.ref('Products2').once('value', (snapshot) => {
        snapshot.forEach((childSnapshot) => {
            ...
            if (selectedCategory === '' || selectedCategory === categoryId) {
                ...
                database.ref('Categories2/' + categoryId).once('value', (categorySnapshot) => {
...
document.getElementById('newProductForm').addEventListener('submit', function(e) {
    ...
    resizeAndCompressImage(productImageFile, 800, 800, 0.7, function(dataUrl) {
        ...
        // Firebase Storage'a yükleme
        const storageRef = storage.ref('productImages2/' + productImageFile.name);
        const uploadTask = storageRef.put(blob);
        ...
        uploadTask.on('state_changed',
            ...
            () => {
                uploadTask.snapshot.ref.getDownloadURL().then((url) => {
                    const newProductKey = database.ref().child('Products2').push().key;
                    database.ref('Products2/' + newProductKey).set({
                        name_tr: name_tr,
...
function openUpdateModal(productId) {
    ...
    database.ref('Products2/' + productId).once('value', (snapshot) => {
...
function updateProduct(productId, name_tr, description_tr, price, categoryId, imageUrl) {
    database.ref('Products2/' + productId).set({
        name_tr: name_tr,
        description_tr: description_tr,
        price: price,
        categoryId: categoryId,
...
function deleteProduct(productId) {
    ...
    if (confirmDelete) {
        database.ref('Products2/' + productId).remove()
            .then(() => {
